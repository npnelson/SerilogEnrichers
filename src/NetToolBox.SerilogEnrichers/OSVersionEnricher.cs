...
namespace NetToolBox.SerilogEnrichers
{
    public sealed class OSVersionEnricher : ILogEventEnricher
    {
        ...
        {
            return propertyFactory.CreateProperty(LinuxVersionPropertyName, RuntimeInformation.OSDescription);
        }

        [MethodImpl(MethodImplOptions.NoInlining)]
        private static LogEventProperty CreateOSProperty(ILogEventPropertyFactory propertyFactory)
        {
            return propertyFactory.CreateProperty(OSPropertyName, NETToolBox.LinuxVersion.GetLinuxVersion.GetLinuxVersionInfo().VersionString);
        }
    }
}
